...
function Layout.open(buffers)
  ...
  local browse_width = math.max(math.floor(total_width * options.layout.browse_width), options.layout.min_browse_width)
  ...
  local right_height = vim.api.nvim_win_get_height(todo_win) + vim.api.nvim_win_get_height(input_win)
  local top_height = math.max(math.floor(right_height * options.layout.todo_height), options.layout.min_sidebar_height)
